...
class Query(object):
    ...
    def fetch_batch_of_results(self):
        uri = "/evaluation/submission/query?query=" + urllib.quote_plus(
            "%s limit %s offset %s" % (self.query, self.limit, self.offset))
...
def list_teams_OLD(evaluation):
    print evaluation
    if type(evaluation) != Evaluation:
        evaluation = syn.getEvaluation(evaluation)

    print '\n\nListing Teams ', evaluation.id, evaluation.name
    print "-" * 60
    sys.stdout.flush()
    ...
    for submission, status in syn.getSubmissionBundles(evaluation, status='VALIDATED'):
        ...
        if 'teamId' in submission:
            ...
            if 'name' in team:
                ...
                team_name = submission.teamId
        elif 'userId' in submission:
            team_id_type = 'user'
            team_id = submission['userId']
            profile = syn.getUserProfile(submission.userId)
            team_name = get_user_name(profile)
        else:
            assert False
        print status.status, team_id_type, team_id, team_name
        teams.add((team_id_type, team_id, team_name))

    print "="*80
    print "\t".join(('team_id_type', 'team_id', 'team_name'))
    for x in teams:
        print "\t".join(x)
    return
    ...
    for submission, status in syn.getSubmissionBundles(evaluation, status='RECEIVED'):
        ...
        try:
            submission = syn.getSubmission(
                submission, 
                downloadLocation=conf.SUBMISSION_DIR, 
                ifcollision='overwrite.local'
            )
            score, message = conf.score_submission(evaluation, submission, previous_submissions)
        ...
        except conf.InputError as ex0:
            sys.stderr.write('\n\nInputError scoring submission %s %s:\n' % (submission.name, submission.id))
            validation_message = str(ex0)
            sys.stderr.write('%s\n\n' % validation_message)
            st = StringIO()
...
def list_teams(evaluation):
    ...
    team_submissions = defaultdict(lambda: defaultdict(list))
    for submission, status in syn.getSubmissionBundles(evaluation, status='VALIDATED'):
        if 'teamId' in submission:
            team_id_type = 'team'
            team_id = submission['teamId']
            ...
            assert False
        x = syn.restGET('/evaluation/submission/{id}'.format(id=submission.id))
        fname = json.loads(x['entityBundleJSON'])['fileHandles'][0]['fileName']
        timestamp_pat =  "%Y-%m-%dT%H:%M:%S"# 2016-09-06T00:56:03.769Z
        submission_time = datetime.strptime(submission.createdOn.split(".")[0], timestamp_pat)
        timestamp = int((submission_time-datetime(1970, 1, 1)).total_seconds())
        local_fname = "/srv/scratch/nboley/encode/final/submissions/{team_id}.{submission_id}.{timestamp}.{fname}".format(
            team_id=team_id, submission_id=submission.id, timestamp=timestamp, fname=fname)
        submission = syn.getSubmission(submission)
        shutil.copyfile(submission.filePath, local_fname)
        print team_id, team_name, submission.id, submission.createdOn, fname, local_fname, os.path.exists(local_fname)
        #teams.add((team_id_type, team_id, team_name))
...
def command_reset(args):
    if args.rescore_all:
        rescore_status = None if args.rescore_invalid else 'SCORED'
        for queue_info in conf.evaluation_queues:
            for submission, status in syn.getSubmissionBundles(queue_info['id'], status=rescore_status):
        ...
        for queue_id in args.rescore:
            for submission, status in syn.getSubmissionBundles(queue_id, status=rescore_status):
                status.status = args.status
                if args.dry_run:
...
def command_validate(args):
    raise NotImplementedError('Validation is built into scoring')
    # if args.all:
    #     for queue_info in conf.evaluation_queues:
...
def command_list_teams(args):
    if args.evaluation:
        list_teams(args.evaluation)
    else:
        sys.stderr.write("List teams command requires an evaluation ID")
...
def main():
    ...
    parser_reset.add_argument("--rescore-invalid", action="store_true", default=False)
    ...
    parser_score = subparsers.add_parser('listteams', help="List all teams that have submitted to an evaluation")
    parser_score.add_argument("evaluation", metavar="EVALUATION-ID", nargs='?', default=None)
    parser_score.set_defaults(func=command_list_teams)
...
if __name__ == '__main__':
    syn = synapseclient.Synapse()
    syn.login(email='nboley', password='')
    list_teams(7373880)
    #main()
